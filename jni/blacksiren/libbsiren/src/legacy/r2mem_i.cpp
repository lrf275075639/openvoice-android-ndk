...
int r2mem_i::process(char* pData_In, int iLen_In, float**& pData_Out, int& iLen_Out){
    ...
    if (m_iInType == r2_in_int_16) {
        
        assert(iLen_In % (sizeof(short) * m_pMicInfo_In->iMicNum) == 0 );
        iLen_Out = iLen_In / (sizeof(short) * m_pMicInfo_In->iMicNum) ;
        ...
        for (int j = 0 ; j < m_pMicInfo_In->iMicNum ; j ++) {
            ...
            for (int i = 0 ; i < iLen_Out ; i ++) {
              ...
              //  m_pData_Out[iMicId][i] *= 64.0f;
            }
        }
        
        pData_Out = m_pData_Out ;
        
    }else if (m_iInType == r2_in_int_24) {
